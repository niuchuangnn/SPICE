...
class WideResNet_cifar100(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=1, bn_momentum=0.1, leaky_slope=0.0, dropRate=0.0, feature_only=False, test=True, **kwargs):
        super(WideResNet_cifar100, self).__init__()
        nChannels = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor]
        assert ((depth - 4) % 6 == 0)
        n = (depth - 4) // 6
        block = BasicBlock
        ...
        if not feature_only:
            if self.test:
                ...
                self.mlp = nn.Sequential(nn.Linear(self.nChannels, self.nChannels), nn.ReLU(), nn.Linear(self.nChannels, num_classes))
